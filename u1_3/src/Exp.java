...
public class Exp {
    ...
    public String toString() {
        ...
        // print the first positive term
        int positiveId = -1;
        for (int i = 0; i < terms.size(); i++) {
            ArrayList<Factor> fs = terms.get(i).getArr();
            Factor f0 = fs.get(0);
            if (f0 instanceof Const) {
                if (f0.getIndex().compareTo(BigInteger.ZERO) > 0) {
                    outbuilder.append(terms.get(i).toString());
                    positiveId = i;
                    break;
                }
            }
        ...
        // print other terms
        for (int i = 0; i < terms.size(); i++) {
            if (positiveId == -1 || i != positiveId) {
                Factor f0 = terms.get(i).getArr().get(0);
                if (f0 instanceof Const) {
                    if (f0.getIndex().compareTo(BigInteger.ZERO) > 0) {
                        outbuilder.append("+").append(terms.get(i).toString());
                    } else if (f0.getIndex().compareTo(BigInteger.ZERO) < 0) {
                        outbuilder.append(terms.get(i).toString());
                    }
                }
            }
    ...
    public void simplify() {
        // delete the bracket in expfactor
        // merge const, pow in a term
        for (Term t : terms) {
            t.recursionSimplify();
        }
        // delete the term whose const = 0
        for (int i = terms.size() - 1; i >= 0; i--) {
            Term t = terms.get(i);
            Factor f = t.getArr().get(0);
            if (f instanceof Const) {
                if (((Const) f).getIndex().equals(BigInteger.ZERO)) {
                    terms.remove(i);
                }
            }
        }
        //        // merge const in diff terms
